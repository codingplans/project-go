...
# -为了避免对市场造成冲击，我们对流动性进行控制，每日买入股票的总数量需要控制在500股以内。当受到控制时，优先保障买入当
# 前（当日买入前）落后总买入计划最多的股票，如果落后数量相同，优先保障A股票的交易，然后是B股票，最后是C股票。
# -每只股票都会发生某日停牌不能交易的情况。当发生这种情况的时候，当日无法买入。
# -当某只股票由于停牌或者流动性控制导致没有按照预期足额买入时，买入量开始落后于总买入计划。所以下一次的买入数量增加原始
# 日计划买入量的30%。加速可以叠加，但买入量不能超过总计划的进度。当买入量追赶上原总计划后恢复到按原计划执行。
